...
use axia_wasm::elements::{External, FunctionType, Internal, Module, Type, ValueType};
...
fn main() {
    ...
    let loaded_module = wasmi::Module::from_axia_wasm_module(module).expect("Module to be valid");

    // Intialize deserialized module. It adds module into It expects 3 parameters:
    // - a name for the module
    // - a module declaration
    // - "main" module doesn't import native module(s) this is why we don't need to provide external native modules here
    // This test shows how to implement native module https://github.com/NikVolf/axia-wasm/blob/master/src/interpreter/tests/basics.rs#L197
...
fn load_module(file: &str) -> Module {
    axia_wasm::deserialize_file(file).expect("File to be deserialized")
}

#[cfg(not(feature = "std"))]
fn load_module(file: &str) -> Module {
    let mut buf = std::fs::read(file).expect("Read file");
    axia_wasm::deserialize_buffer(&mut buf).expect("Deserialize module")
}
