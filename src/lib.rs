...
pub struct Module {
    ...
    module: axia_wasm::elements::Module,
}

impl Module {
    /// Create `Module` from `axia_wasm::elements::Module`.
    ///
    /// This function will load, validate and prepare a `axia_wasm`'s `Module`.
    ///
    /// # Errors
    ///
    ...
    ///         .build();
    ///
    ///     let module = wasmi::Module::from_axia_wasm_module(axia_module)
    ///         .expect("axia-wasm builder generated invalid module!");
    ///
    ///     // Instantiate `module`, etc...
    /// }
    /// ```
    pub fn from_axia_wasm_module(module: axia_wasm::elements::Module) -> Result<Module, Error> {
    ...
    pub fn from_buffer<B: AsRef<[u8]>>(buffer: B) -> Result<Module, Error> {
        let module = axia_wasm::elements::deserialize_buffer(buffer.as_ref())
            .map_err(|e: axia_wasm::elements::Error| Error::Validation(e.to_string()))?;
        Module::from_axia_wasm_module(module)
    }

    pub(crate) fn module(&self) -> &axia_wasm::elements::Module {
        &self.module
    }
